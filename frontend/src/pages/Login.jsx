import { useState } from "react";
import { useAuth } from "../context/AuthContext";
import "../styles/auth.css";

export default function Login() {
  const { signInWithGoogle } = useAuth();
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);

  const handleGoogleSignIn = async () => {
    setError("");
    setLoading(true);
    try {
      await signInWithGoogle();
    } catch (err) {
      setError(err.message);
      setLoading(false);
    }
  };

  return (
    <div className="auth-page">

      {/* ══ LEFT — Hero ══ */}
      <aside className="auth-left">

        {/* Version pill */}
        <div className="auth-version-pill">
          <span className="auth-version-dot" />
          <span>V1.0 NOW LIVE</span>
        </div>

        {/* Hero body */}
        <div className="auth-hero-body">
          <h1 className="auth-hero-heading">
            Fill any form.<br />
            <em>In one click.</em>
          </h1>

          <p className="auth-hero-sub">
            Store your profile once — name, phone, address, education & more.
            AutoSlay auto-fills any form instantly, so you never retype the same
            details twice.
          </p>

          <div className="auth-cta-row">
            <a
              href="https://chromewebstore.google.com/detail/autoslay-%E2%80%93-smart-form-fil/lafjiaflpnifhglhjcolidljogmbigco"
              target="_blank"
              rel="noreferrer"
              className="auth-cta-primary"
            >
              Install Extension
            </a>
            <a
              href="#demo"
              className="auth-cta-secondary"
            >
              Watch Demo
            </a>
          </div>
        </div>

      </aside>

      {/* ══ RIGHT — Auth card ══ */}
      <section className="auth-right">
        <div className="auth-card">

          {/* Fingerprint icon */}
          <div className="auth-card-fp">
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round">
              <path d="M12 10a2 2 0 0 0-2 2c0 1.02-.1 2.51-.26 4"/>
              <path d="M14 13.12c0 2.38 0 6.38-1 8.88"/>
              <path d="M17.29 21.02c.12-.6.43-2.3.5-3.02"/>
              <path d="M2 12a10 10 0 0 1 18-6"/>
              <path d="M2 16h.01"/>
              <path d="M21.8 16c.2-2 .131-5.354 0-6"/>
              <path d="M5 19.5C5.5 18 6 15 6 12a6 6 0 0 1 .34-2"/>
              <path d="M8.65 22c.21-.66.45-1.32.57-2"/>
              <path d="M9 6.8a6 6 0 0 1 9 5.2v2"/>
            </svg>
          </div>

          <div className="auth-card-header">
            <h2 className="auth-title">Welcome back</h2>
            <p className="auth-subtitle">Access your global profile vault.</p>
          </div>

          {error && <div className="auth-error">{error}</div>}

          <button
            type="button"
            className="google-btn"
            onClick={handleGoogleSignIn}
            disabled={loading}
          >
            {loading ? (
              <span className="spinner" />
            ) : (
              <>
                <svg width="18" height="18" viewBox="0 0 24 24">
                  <path fill="#000" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="#000" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="#000" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                  <path fill="#000" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
                Sign in with Google
              </>
            )}
          </button>

          <div className="auth-card-divider" />

          <div className="auth-card-footer">
            New to the elite tier?{" "}
            <a href="#" onClick={handleGoogleSignIn}>Create Account</a>
          </div>

        </div>
      </section>

    </div>
  );
}
